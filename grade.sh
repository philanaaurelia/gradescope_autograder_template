...
EXPECTED_FILES=("divider.h")
MAKE_TARGET="all"


DIFF_TOOLS=ucsb-gradescope-diff-based-testing
...
if [ -d $DIFF_TOOLS ]; then
  ...
  git clone https://github.com/philanaaurelia/${DIFF_TOOLS}.git
fi
...
# Compile and print any compiler errors to stderr.txt
make clean
rm -f stderr.txt
make $MAKE_TARGET 2> stderr.txt
echo "stderr.txt ERRORs"
cat stderr.txt

# Copy text to parent file to print out to student
cp ./stderr.txt ../
